...
//generer les vehicules en fonction de l'intensite
...
species voiture skills:[moving]
{
	...
	reflex deplacer_parking when: the_target=nil{
		...
		loop i over: Allparking{
			//Recuperation de la taille de chaque parking en considerant son air comme sa taille
			float taille_parking<-i.shape.area;
			write(taille_parking);
		
		//verifie si l'espace du parking est plus grand que la taille de la voiture
		if(taille_parking>size)
		{
		  do action: goto target: the_target_parking  speed:2.0;	
